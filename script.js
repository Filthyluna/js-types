 ...
 */
let removedPeriods = []; //make empty array
for (let i = 0; i < q9.length; i++){ //loop
  removedPeriods[i] = q9[i].replace(".", ""); //fill empty array with old array that has periods removed
}
const q11 = removedPeriods; //set q11 to the filled array
...
for (let i = 0; i < q12.length; i++){
  averageWordLength += q12[i];
}
const q13 = averageWordLength / q12.length;
